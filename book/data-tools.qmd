...
*Data Tools* can be combined (e.g. interpolate followed by detrend) and run before carrying out analyses. When [Batch Mode]{.ui-label} is off, running these methods will clear previous analysis from the graph and results table. When [Batch Mode]{.ui-label} is on, running these methods will clear previous analysis from the graph but not from the results table (and running a new analysis will add new table columns rather than overwrite the previous results).

Interpolation and filtering methods use routines from the [SciPy](https://www.scipy.org/) package.
...
Downsampling reduces the data resolution by discarding every nth sample (up to a factor of 12). Prior to downsampling, the primary channel data is low-pass filtered to the Nyquist frequency of the downsampled data. For two-channel data, both the primary and secondary channel are downsampled, but only the primary channel is filtered before downsampling. This is because it is assumed the secondary channel will contain $I_\mathrm{m}$ and $V_\mathrm{m}$ step protocols. Options for choosing the low-pass filter are provided (default 8^th^ order Bessel).
...
If your file has two channels (i.e. $V_\mathrm{m}$ and $I_\mathrm{m}$) clicking the checkbox option will also calculate and remove the baseline from the second data channel (across the time window specified on the primary data channel).

## [Average Records]{.ui-label}

Averaging across records is useful for reducing noise in repeated-stimulation protocols. If set to [Average All Records]{.ui-label}, every record in the file will be averaged into one record.

Alternatively, specific records can be selected for averaging by choosing the [Average specific records]{.ui-label} option from the dropdown menu. A single-column table will be displayed. The rows of the table can be used to group records for averaging. For example, in a 10-record file 1,2,3 could be entered to the first row, and 4,5,6 into the second. Clicking [Apply]{.ui-label} would generate with 2 records (record 1 - average of 1,2,3; record 2 - average of 4,5,6, with records 7,8,9,10 discarded).

## [Record Arithmetic]{.ui-label}
...
![Record Arithmetic. Pointwise mathematical operations can be applied across records. In this example, the resulting dataset contains two records formed from the sums of adjacent source records.](images/media/image32.png){width="2.2805555555555554in" height="4.417361111111111in"}

## [Stimulus Artefact Removal]{.ui-label}
...
![Stimulus Artefact Removal. Periodic stimulus artefacts can be detected and removed automatically. The orange preview shows the proposed result before it is applied to the trace.](images/media/image33.png){width="6.814987970253719in" height="3.926525590551181in"}
...
## [Reshape Records]{.ui-label}

Selecting [Concatenate all records into a single record]{.ui-label} will combine multiple records into one long record. Note that where recorded time intervals between records are not continuous, because they reflect gaps in the recording schedule (e.g. Record 1, $0$–$2\ \mathrm{s}$, Record 2, $3$–$5\ \mathrm{s}$), they will be made continuous (e.g. record 1 and record 2 would be concatenated into a single record between time points $0$–$4\ \mathrm{s}$).

Alternatively, a single-record dataset can be split into multiple records if the number of records is known by selecting [Expand data from a single record to multiple records]{.ui-label}. This can be useful as in some instances `.abf` files exported from other software (e.g. Igor, WinWCP) are concatenated into a single-record file, or to increase performance for long single-record files.
...
## [Normalize Time]{.ui-label}

The time in electrophysiological data may be recorded in two ways. If normalized, the times of each record are the absolute record length (e.g. for $2\ \mathrm{s}$ recordings, record 1: $0$–$2\ \mathrm{s}$, record 2: $0$–$2\ \mathrm{s}$). Alternatively time may be recorded relative to the start of recording, increasing cumulatively (e.g. record 1: $0$–$2\ \mathrm{s}$, record 2: $2$–$4\ \mathrm{s}$). *Normalize Time* applies the time vector from the first record for every record in the recording. Note that this assumes all records in the dataset are the same length.

## [Cut Trace Length]{.ui-label}

This gives the option to set new record start and finish times (e.g. cut a trace that is $0$–$2\ \mathrm{s}$ long down to the time period $0.5$–$1\ \mathrm{s}$). An offset will always be subtracted to ensure the first record starts at time = 0.

Clicking [Apply]{.ui-label} will change the record length for every record in the file. Options are provided for cumulatively increasing record times after cutting trace length (e.g. record 1: $0$–$0.5\ \mathrm{s}$, record 2: $0.5$–$1\ \mathrm{s}$) or normalizing time instead (see *Normalize Time*).

## [Reset to Raw Data]{.ui-label}
