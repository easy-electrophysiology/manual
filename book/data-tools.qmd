...
Downsampling reduces the data resolution by discarding every nth sample (up to a factor of 12). Prior to downsampling, the primary channel data is low-pass filtered to the Nyquist frequency of the downsampled data. For two-channel data, both the primary and secondary channel are downsampled, but only the primary channel is filtered before downsampling. This is because it is assumed the secondary channel will contain Im and Vm step protocols. Options for choosing the low-pass filter are provided (default 8^th^ order Bessel).
...
![Filter Data. Data are low-pass filtered at 1000 Hz using an eighth-order Bessel filter. [See Filtered Data]{.ui-label} overlays the filtered trace for inspection before applying the filter.](images/media/image31.PNG){width="6.268055555555556in" height="3.752083333333333in"}
...
This gives the option to set new record start and finish times (e.g. cut a trace that is 0 -- 2 s long down to the time period 0.5 -- 1 s). An offset will always be subtracted to ensure the first record starts at time = 0.

Clicking [Apply]{.ui-label} will change the record length for every record in the file. Options are provided for cumulatively increasing record times after cutting trace length (e.g. record 1: 0 -- 0.5 s, record 2: 0.5 -- 1 s) or normalizing time instead (see *Normalize Time*).
